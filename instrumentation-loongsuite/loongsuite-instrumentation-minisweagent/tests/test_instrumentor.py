...
class TestImportAndMetadata:
    ...
    def test_supports_metrics(self):
        from opentelemetry.instrumentation.minisweagent.package import _supports_metrics
...
class TestGetEnvironmentWrapping:
    ...
    def test_wrapped_get_environment_returns_tracing_environment(self):
        from opentelemetry.instrumentation.minisweagent import MiniSweAgentInstrumentor
        from opentelemetry.instrumentation.minisweagent.internal.delegates import TracingEnvironment
        import minisweagent.environments as envs_mod
...
class TestAgentWrapperHelpers:
    ...
    def test_task_preview_short_string(self):
        from opentelemetry.instrumentation.minisweagent.internal.agent_wrappers import _task_preview
    ...
    def test_request_model_from_agent(self):
        from opentelemetry.instrumentation.minisweagent.internal.agent_wrappers import _request_model_from_agent
...
class TestLimitsExceeded:
    ...
    def test_no_config(self):
        from opentelemetry.instrumentation.minisweagent.internal.agent_wrappers import DefaultAgentStepWrapper
...
class TestTracingEnvironment:
    ...
    def test_getattr_delegates_to_inner(self, stub_environment):
        ...
        te = TracingEnvironment(stub_environment, tracer)
        assert te.custom_attr == "hello"
    ...
    def test_execute_delegates_and_returns_result(self, stub_environment):
        ...
        tracer = trace_api.get_tracer("test")
        te = TracingEnvironment(stub_environment, tracer)
        result = te.execute({"command": "ls"}, cwd="/tmp")
    ...
    def test_execute_handles_non_dict_result(self):
        ...
        tracer = trace_api.get_tracer("test")
        te = TracingEnvironment(NonDictEnv(), tracer)
        result = te.execute({"command": "echo"})
    ...
    def test_execute_propagates_exception(self):
        ...
        tracer = trace_api.get_tracer("test")
        te = TracingEnvironment(FailEnv(), tracer)
        with pytest.raises(RuntimeError, match="command failed"):
            te.execute({"command": "fail"})
    ...
    def test_execute_propagates_interrupt_agent_flow(self):
        ...
        tracer = trace_api.get_tracer("test")
        te = TracingEnvironment(InterruptEnv(), tracer)
        with pytest.raises(InterruptAgentFlow):
            te.execute({"command": "interrupt"})
    ...
    def test_execute_with_non_dict_action(self):
        ...
        tracer = trace_api.get_tracer("test")
        te = TracingEnvironment(AnyEnv(), tracer)
        # Non-dict action: command extraction should default to ""
        result = te.execute("not a dict")
...
class TestSanitizeToolResult:
    ...
    def test_normal_dict(self):
        from opentelemetry.instrumentation.minisweagent.internal.delegates import _sanitize_tool_result
...
class TestMiniTyperAppProxy:
    ...
    def test_proxy_delegates_call(self):
        from opentelemetry.instrumentation.minisweagent.internal.cli_wrappers import _MiniTyperAppProxy
