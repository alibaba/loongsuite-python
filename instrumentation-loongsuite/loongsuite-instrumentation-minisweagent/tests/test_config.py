...
class TestModuleLevelConstants:
    ...
    def test_task_preview_env_override_via_int_env(self):
        ...
        with patch.dict(os.environ, {"OTEL_MINISWEAGENT_TASK_PREVIEW_MAX_LEN": "512"}):
            assert _int_env("OTEL_MINISWEAGENT_TASK_PREVIEW_MAX_LEN", "256") == 512
    ...
    def test_command_preview_env_override_via_int_env(self):
        ...
        with patch.dict(os.environ, {"OTEL_MINISWEAGENT_COMMAND_PREVIEW_MAX_LEN": "128"}):
            assert _int_env("OTEL_MINISWEAGENT_COMMAND_PREVIEW_MAX_LEN", "256") == 128

    def test_task_preview_invalid_env_via_int_env(self):
        """Non-integer env var should fall back to 256."""
        from opentelemetry.instrumentation.minisweagent.config import _int_env

        with patch.dict(os.environ, {"OTEL_MINISWEAGENT_TASK_PREVIEW_MAX_LEN": "abc"}):
            assert _int_env("OTEL_MINISWEAGENT_TASK_PREVIEW_MAX_LEN", "256") == 256
