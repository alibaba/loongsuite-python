...
def _instrument(tracer_provider, meter_provider, logger_provider, mode):
...
def _uninstrument(instrumentor, span_exporter):
...
def instrument(
    ...
    instrumentor = _instrument(
        tracer_provider, meter_provider, logger_provider, "SPAN_ONLY"
    )
    yield instrumentor
    _uninstrument(instrumentor, span_exporter)
...
def instrument_no_content(
    ...
    instrumentor = _instrument(
        tracer_provider, meter_provider, logger_provider, "NO_CONTENT"
    )
    yield instrumentor
    _uninstrument(instrumentor, span_exporter)
...
def reset_dspy_settings():
    """Keep DSPy global settings from leaking between tests."""
    yield
    dspy.settings.configure(lm=None, rm=None, track_usage=False)
